        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h5>Search Users by Email and Name</h5>
                ...
                <div class="card-block row custon-select" style="display: none;">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="exampleFormControlSelect1">Search by Username/Email</label>
                            <input class="form-control filter-search-control" id="email-search" name="email_search" type="search" placeholder="Search...">
                            <button type="submit" id="email-search-submit" class="custon-select-icon"><i class="fa fa-search"></i></button>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="exampleFormControlSelect1">Search by Name</label>
                            <input class="form-control filter-search-control" id="name-search" name="name_search" type="search" placeholder="Search...">
                            <button type="submit" id="name-search-submit" class="custon-select-icon"><i class="fa fa-search"></i></button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#tabs-nav').on('click', 'a', function(event) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    switch (filter) {
                        case 'all':
                            $('#all-users').html(data.user_list);
                            $('#all-users').append(data.users_pagination_links);
                            break;
                        case 'inactive':
        ...
        /** Handle Pagination For Users Links Init and Ajax Loading
         *  On click Pagination Links Load 
         * 
         */
    ...
    $('#email-search-submit').on('click', function(event) {
        event.preventDefault();
        var emailSearchVal = $('#email-search').val();
        console.log(emailSearchVal);
        $.ajax({
            url: "{{url('acl/users/list/search-by-email')}}",
            method: 'GET',
            data: {
                email: emailSearchVal
            },
            success: function(data) {
                console.log(data)
                $('#all-users').html(data.user_list);
    ...
    $('#name-search-submit').on('click', function(event) {
        event.preventDefault();
        var nameSearchVal = $('#name-search').val();
        console.log(nameSearchVal);
        $.ajax({
            url: "{{url('acl/users/list/search-by-name')}}",
            method: 'GET',
            data: {
                name: nameSearchVal
            },
